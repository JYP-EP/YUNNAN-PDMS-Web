...
stopDocker
set -ex
image=192.168.8.205:5000/iot:std
...
docker run -t  -e WORKSPACE=/home -e GITNUM=$gitversion -e VERSION=$version   -v ${WORKSPACE}:/home   --privileged=true --rm --name ${JOB_NAME} ${image} scripts/build.sh
