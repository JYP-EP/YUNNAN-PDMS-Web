...
<script>
import {getAnalogInfo, localDateStringFromEpoch} from "@/api/gridnt/monitor";
...
export default {
  ...
  created() {
    console.log('analog created');
  },
  mounted() {
    console.log('analog mounted');
  },
  methods:{
    ...
    deviceClicked(data){
      ...
      getAnalogInfo(data.id).then(res => {
    ...
    initTarget(){
      // query analog data from mysql type <= 900 or type > 950
    },
    updateTarget(){
      // update analog data from realtime data
    }
  }
}
</script>
