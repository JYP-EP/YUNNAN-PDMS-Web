        ...
        <el-table max-height="700" :data="tableData" stripe>
          ...
          <el-table-column align="center" prop="status" label="操作">
            <template slot-scope="scope">
              <trans-current-xml :alert-item="scope.row" v-if="scope.row.alertType === 5"></trans-current-xml>
...
<script>
  import {getTransformerCurrentAlert} from '@/api/gridnt/intelAlert';
  ...
  export default {
    ...
    components: {
      RawFileContent,
      CompareXml, DoubleAnalogXml, AnalogXml, SecCircuitXml, CommRatioXml, frequentXml, Pagination,OuterRimXml,TransCurrentXml
    },
    computed: {
      operatorOptions() {
    ...
    methods: {
      ...
      //获取到区外分析告警的数据
      requestAlert() {
        getTransformerCurrentAlert(this.queryParams).then(resp => {
          if (resp.data) {
