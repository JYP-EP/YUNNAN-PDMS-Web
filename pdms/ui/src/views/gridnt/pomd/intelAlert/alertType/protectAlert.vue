    ...
    <el-row :gutter="24" style="width: 100%">
      <el-col :span="17">
        ...
        <el-table max-height="700" :data="tableData" style="width: 100%">
          ...
          <el-table-column align="center" prop="alertLevel" label="危机类型"
                           :formatter="alertLevelParser"></el-table-column>
          <el-table-column align="center" prop="status" label="操作">
            <template slot-scope="scope">
              <tunnel-xml :alert-item="scope.row"></tunnel-xml>
            </template>
          </el-table-column>
        </el-table>
        <pagination
...
<script>
  ...
  import {getProtectTunnelAlert, getIntelAlertLevel} from "@/api/gridnt/intelAlert";
  ...
  export default {
    ...
    components: {TunnelXml, Pagination},
    data() {
      return {
        queryParams: {
          page: {
    ...
    methods: {
      ...
      requestAlert() {
        getProtectTunnelAlert(this.queryParams).then(resp => {
          if (resp.data) {
        ...
        getIntelAlertLevel(this.queryParams).then(resp => {
          if (resp.data) {
      ...
      updateEcharts(statData) {
        ...
        statData.forEach(item => {
          ...
          levelMap.get(item.alertLevel)[index] = item.frequency;
        });
