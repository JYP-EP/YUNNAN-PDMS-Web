        ...
        <el-table max-height="700" :data="tableData" stripe>
          ...
          <el-table-column align="center" prop="status" label="操作">
            <template slot-scope="scope">
              <check-report-xml :alert-item="scope.row"></check-report-xml>
            </template>
          </el-table-column>
...
import {getCheckAlert} from '@/api/gridnt/intelAlert';
...
export default {
  ...
  methods:{
    ...
    requestAlert(){
      getCheckAlert(this.queryParams).then(resp=>{
        if(resp.data){
