        ...
        <el-table max-height="700" :data="tableData" stripe>
          ...
          <el-table-column align="center" prop="status" label="操作">
            <template slot-scope="scope">
              ...
              <G_analogXml :alert-item="scope.row" v-if="scope.row.alertType === 11"></G_analogXml>
...
<!--              <compare-xml :alert-item="scope.row" v-if="scope.row.alertType === 13"></compare-xml>-->
              <G_SettingValueAlert :alert-item="scope.row" v-if="scope.row.alertType === 13"></G_SettingValueAlert>
            </template>
          </el-table-column>
...
import {getStatAlert} from '@/api/gridnt/intelAlert';
...
export default {
  ...
  components:{
    RawFileContent,
    CompareXml, DoubleAnalogXml, AnalogXml, SecCircuitXml, CommRatioXml, frequentXml, Pagination,
    G_SecCircuit, G_SettingValueAlert, G_analogXml},
  data() {
    return {
      queryParams: {
        page: {
  ...
  methods:{
    ...
    requestAlert(){
      getStatAlert(this.queryParams).then(resp=>{
        if(resp.data){
